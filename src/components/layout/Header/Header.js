  ...
  return (
    <MainHeader>
      <Container className="flex items-center justify-between w-full h-full py-2">
        <Linked
          href="/"
          className="h-full w-12 shrink-0 uppercase relative text-white">
          <Image src="/iso.svg" layout="fill" width={100} height={100} />
        </Linked>
        ...
        <nav className="hidden md:flex px-3">
          <ul className="flex flex-1 items-center space-x-1">
            {LINKS.map(({ title, route }, index) => (
              <li
                key={`nav-top-${index}`}
                className="relative px-2 uppercase text-sm font-medium">
        ...
        <Linked
          href="/contacto"
          className="px-2 uppercase text-sm font-medium hidden md:block">
          contáctanos
        </Linked>
